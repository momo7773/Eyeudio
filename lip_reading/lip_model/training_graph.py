...
class TransformerTrainGraph():
    def __init__(self,
        ...
        self.decoder_inputs = tf.concat( (tf.ones_like(self.prev[:, :1]) * go_token_index, self.prev[:, :-1]), -1 )  # 2:<S>
        ...
        if chars is not None:
            ...
            self.char_table = tf.lookup.StaticHashTable(tf.lookup.KeyValueTensorInitializer(keys, values), '')
    ...
    def positional_encoding(self, inp, scope):
        if config.sinusoid:
            ...
            return embedding(
                tf.tile(tf.expand_dims(tf.range(tf.shape(inp)[1]), 0), [tf.shape(inp)[0], 1]),
    ...
    def add_loss_and_metrics(self, reuse, is_training):
        ...
        if self.project_output():
            ...
            assert(self.dec.get_shape()[-1] == config.n_labels)
            self.logits = self.dec

        if config.test_aug_times:
            self.logits_aug = self.logits
            self.logits = tf.reduce_mean(self.logits, 0, keepdims=True)
        ...
        if not 'infer' in config.graph_type:
            ...
            self.preds = tf.cast(self.preds, tf.int32)
    ...
    def add_tb_summaries(self):
        ...
        if not config.test_aug_times:
            ...
            add_gif_summary(
                '2-input_to_resnet',
                tf.concat([ self.visual_frontend.aug_out[b_id][fr_in:to_in] for b_id in xrange(bs) ], axis=2),
                fps=fps, timeline=timeline
            )

        # ----------------   Add text summaries -------------------------------
        pred_strings_tf = self.char_table.lookup(tf.cast(self.preds, tf.int64))
        joined_pred = tf.compat.v1.string_join(tf.split(pred_strings_tf, pred_strings_tf.shape[1], 1))[ :, 0]
        gt_strings_tf = self.char_table.lookup(tf.cast(self.y, tf.int64))
        joined_gt = tf.compat.v1.string_join(tf.split(gt_strings_tf, pred_strings_tf.shape[1], 1))[:, 0]
    ...
    def get_model_input_target_shapes_and_types(cls, batch_dims=1):
        return cls.get_input_shapes_and_types(batch=batch_dims), cls.get_target_shapes_and_types(batch=batch_dims)
