...
from .beam_search import log_prob_from_logits
from .training_graph import TransformerTrainGraph
from .modules import embedding, multihead_attention, feedforward
...
class TransformerInferenceGraph(TransformerTrainGraph):
    ...
    def decoder_body(self, enc, dec, is_training, top_scope=None):
        ...
        if config.beam_size > 1:  # Beam Search
            ...
            if config.top_beams == 1:
                ...
                if StrictVersion(tf.__version__) >= StrictVersion('1.5'):
                    decoded_ids.decoded_ids.__dict__['_shape_val'] = tf.TensorShape([logits_bs, config.top_beams, None])
                else:
            ...
            def inner_loop(i, next_id, decoded_ids, decoded_logits, cache):
                ...
                next_id = tf.cast(tf.argmax(combined_scores, axis=-1), tf.int64)
    ...
    def initialize_cache(self, batch_size, enc):
        ...
        for layer in cache:
            if StrictVersion(tf.__version__) >= StrictVersion('1.5'):
                cache[layer]["k"].__dict__['_shape_val'] = tf.TensorShape( [None, None, key_channels])
                cache[layer]["v"].__dict__['_shape_val'] = tf.TensorShape( [None, None, value_channels])
            else:
    ...
    def get_symbols_to_logits_fun(self, enc, timing_signal, is_training, top_scope):
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            if config.test_aug_times:
                targets = tf.tile(tf.expand_dims(targets, axis=0), [config.test_aug_times] + [1] * (len(targets.shape)))
            ...
            with tf.compat.v1.variable_scope(top_scope, reuse=tf.compat.v1.AUTO_REUSE):
                logits = tf.compat.v1.layers.dense(pre_logits, config.n_labels, reuse=reuse_loop)
        ...
        def preprocess_targets(targets, i, reuse_loop):
            ...
            ## Dropout
            targets = tf.compat.v1.layers.dropout(targets,
    ...
    def get_lm_symbols_to_logprobs_handle(self, cache, logits_bs, top_scope):
        ...
        lip_to_lm_chars = dict( [(self.char_inds[chr], lm_idx) for chr, lm_idx in rnn_dict['char_inds'].items() if chr in self.char_inds] )
        ...
        def lm_symbols_to_logprobs_fn(ids, unused_i, cache, inf_mask_tf=zero_mask_tf):
            ...
            with tf.control_dependencies([tf.assert_non_negative(lm_ids, [lm_ids])]):
                with tf.compat.v1.variable_scope(top_scope, reuse=True): # we have initialized LM outside
