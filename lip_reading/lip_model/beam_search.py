...
def _merge_batch_into_time_dim(tensor):
    ...
    return tf.reshape( tf.transpose(tensor, [1,0,2,3]), [1, beam_size, batch_size * t, c ])
...
def log_prob_from_logits(logits, axis=2):
    return logits - tf.compat.v1.reduce_logsumexp(logits, axis=axis, keepdims=True)
...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
                                beam_size, batch_size, batch_size_states, prefix="default",
    ...
    if states_to_gather:
        topk_gathered_states = gather_nd_states(states_to_gather,
                                                top_coordinates,
                                                batch_size_states,
...
def beam_search(symbols_to_logits_fn,
    ...
    def grow_finished(finished_seq, finished_scores, finished_flags, 
        ...
        # finished scores
        finished_seq = tf.concat( [finished_seq, tf.zeros([batch_size, beam_size, 1], tf.int64)], axis=2)
        ...
        return compute_topk_scores_and_seq(
            ...
            curr_finished_flags, beam_size, batch_size, batch_size_states, "grow_finished"
        )
    ...
    def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished, states):
        ...
        return compute_topk_scores_and_seq(curr_seq, curr_scores, curr_log_probs,
                                           ...
                                           batch_size_states, "grow_alive", states)
    ...
    def grow_topk(i, alive_seq, alive_log_probs, states):
        ...
        if states:
            ...
            if not lm_symbols_to_logprobs_fn is None:
                flat_lm_logprobs, flat_states = lm_symbols_to_logprobs_fn(flat_ids,i, flat_states)
        ...
        if not lm_symbols_to_logprobs_fn is None:
            ...
            candidate_log_probs += lm_alpha * lm_logprobs # add the lm logprobs to the data ones
    ...
    def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
        ...
        # finished item.
        lowest_score_of_fininshed_in_finished = tf.reduce_min(finished_scores * tf.to_float(finished_in_finished), axis=1)
        # If none of the sequences have finished, then the min will be 0 and
        # we have to replace it by -ve INF if it is. The score of any seq in alive
        # will be much higher than -ve INF and the termination condition will not
        # be met.
        lowest_score_of_fininshed_in_finished += ((1. - tf.to_float(tf.reduce_any(finished_in_finished, 1))) * - INF)
    ...
    (_, alive_seq, alive_log_probs, finished_seq, finished_scores, finished_flags, _) = tf.while_loop(
        _is_finished,
        inner_loop, [
