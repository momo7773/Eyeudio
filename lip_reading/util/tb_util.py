...
def add_gif_summary(name,
    ...
    with summary_op_util.summary_scope(name, family, values=[im_thwc]) as (tag, scope):
        ...
        gif_summ = tf.numpy_function(py_encode_gif, pyfunc_args, tf.string)
        summary_op_util.collect(gif_summ, collections, [tf.compat.v1.GraphKeys.SUMMARIES])
    return gif_summ
...
def py_encode_gif(im_thwc, tag, fps=4, timeline=False, attention=[], preds=[]):
    ...
    if im_thwc.shape[-1] == 1:
        import cv2
        im_thwc = np.array([cv2.cvtColor(gray_img, cv2.COLOR_GRAY2RGB) for gray_img in im_thwc])
